...
def process_swap_shop(device):
    ...
    while running:
        try:
            ...
            # 1. ตรวจสอบ heroo1.png
            hero_pos = ImgSearchADB(adb_img, 'img/heroo1.png')
            if hero_pos:
                print(f"\nพบ heroo1.png บนอุปกรณ์ {device.serial} - ดำเนินการ backup ทันที")
...
def backup_failed_game_data(device):
    """Backup game data to random-Fail folder when heroo1.png is not found"""
    try:
        ...
        try:
            ...
            if result.returncode == 0 and os.path.exists(backup_path) and os.path.getsize(backup_path) > 0:
                print(f"\nDevice {device.serial}: === Backup ไฟล์ที่ไม่พบ heroo1.png สำเร็จ ===")
...
def check_for_hero1(device):
    ...
    try:
        print(f"กำลังค้นหา heroo1.png บนอุปกรณ์ {device.serial}")
        cap = device.screencap()
        image = np.frombuffer(cap, dtype=np.uint8)
        adb_img = cv2.imdecode(image, cv2.IMREAD_COLOR)
        ...
        hero_pos = ImgSearchADB(adb_img, 'img/heroo1.png')
        if hero_pos:
            print(f"พบ heroo1.png บนอุปกรณ์ {device.serial}")
            return True
        return False
    except Exception as e:
    ...
    while True:
        try:
            ...
            if check_for_hero1(device):
                ...
                log_status("ไม่พบ heroo1.png - ดำเนินการต่อ")
